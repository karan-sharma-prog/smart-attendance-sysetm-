    ...
    <div className="p-8 space-y-8">
      <header className="flex justify-between items-center">
        ...
        <div className="flex gap-3">
          ...
          <button className="px-4 py-2 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 flex items-center gap-2">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center">
              ...
              <div className="flex gap-2">
                {['ALL', 'STUDENTS', 'TEACHERS'].map(t => (
                  <button key={t} className="px-3 py-1 text-[10px] font-bold rounded-full bg-slate-100 text-slate-500 hover:bg-indigo-50 hover:text-indigo-600 transition-all uppercase">
                    {t}
                  </button>
                ))}
              </div>
                  ...
                  {[
                    ...
                    <tr key={i} className="hover:bg-slate-50/30">
                      ...
                      <td className="px-6 py-4 text-right">
                        <button className="p-1.5 text-slate-400 hover:text-indigo-600 transition-all">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={i} className="p-4 rounded-xl border border-slate-100 bg-slate-50/50">
                  ...
                  <p className="text-xs text-slate-500">Instructor: <span className="text-indigo-600 font-medium">{c.teacher}</span></p>
                </div>
              ))}
              <button className="w-full py-3 text-xs font-bold text-indigo-600 border border-indigo-200 rounded-xl hover:bg-indigo-50 transition-all">
                MANAGE ALL CLASSES
              </button>
            </div>
          </div>

          <div className="glass-card p-6 rounded-2xl shadow-sm bg-indigo-600 text-white">
            <h3 className="font-bold mb-2">System Health</h3>
            <p className="text-xs text-indigo-100 mb-6">AI Service & Database Status</p>
            <div className="space-y-4">
