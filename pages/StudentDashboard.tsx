          ...
          <div className="glass-card p-6 rounded-2xl shadow-sm text-center">
            ...
            <div className="relative inline-flex items-center justify-center p-4">
              <svg className="w-32 h-32 transform -rotate-90">
                ...
                <circle cx="64" cy="64" r="58" stroke="currentColor" strokeWidth="12" fill="transparent" strokeDasharray={364.4} strokeDashoffset={364.4 - (364.4 * (user.attendancePercentage || 0) / 100)} className={`${(user.attendancePercentage || 0) < 75 ? 'text-red-500' : 'text-indigo-600'} transition-all duration-1000`} />
              </svg>
              ...
              {upcomingClasses.map((cls) => (
                <div key={cls.id} className="p-4 rounded-xl bg-slate-50 border border-slate-100">
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    ...
                    <button className="text-xs font-bold text-indigo-600 hover:underline">RESOURCES →</button>
                  </div>
                </div>
              ))}
            </div>
                ...
                {[
                  ...
                  <tr key={i} className="hover:bg-slate-50/50 transition-all">
                    ...
                    <td className="px-6 py-4">
                      <span className="px-2 py-1 bg-indigo-50 text-indigo-600 rounded text-[10px] font-bold">{row.method}</span>
                    </td>
                    <td className="px-6 py-4">
